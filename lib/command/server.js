...
module.exports = function(options, callback) {
    ...
    app.get(/^\/(save|get)\/([a-z]+)/i, function(req, res) {
        var action = req.params[1].replace(/\/$/, '');
        var extension = '.' + req.params[0] + '.js';
        var handler_module = node_path.join( HANDLER_ROOT, action + extension );
        var option_module = node_path.join( OPTION_ROOT, action + extension );
        ...
        if(fs.exists(handler_module)){
            ...
            handler(options, function(err, data) {
                ...
                res.jsonp(data.code || err && 500 || 200, data.json);
            });
